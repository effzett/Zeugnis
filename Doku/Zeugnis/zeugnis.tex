...
Abbildung~\ref{Schulklassen} auf Seite~\pageref{Schulklassen} zeigt die Dialogmaske für die Eingabe der Schüler.
...
Ist zu jedem Indikator eine Bewertung erfolgt (Bewertungsfeld $!=$ leer) erscheint ein Häkchen neben dem Schülernamen, der anzeigt, dass das Zeugnis nun komplett ist und gedruckt werden kann.

Abbildung~\ref{Zeugnisse} auf Seite~\pageref{Zeugnisse} zeigt die Eingabe der Bewertungen für die Lernbereiche \emph{Arbeits-- und Sozialverhalten}.
Abbildung~\ref{Zeugnisse2} auf Seite~\pageref{Zeugnisse2} zeigt die Eingabe der Bewertungen für die anderen Lernbereiche.
...
Abbildung~\ref{Konfiguration} auf Seite~\pageref{Konfiguration} zeigt die Eingabe der Bewertungen für die anderen Lernbereiche.

\subsubsection{Änderungen in {\ott config.properties}}
\label{configKlassen}
Ind der Konfigurationsdatei {\ott config.properties} können einige Werte des Programms konfiguriert werden, die dann beim nächsten Start des Programms initial verwendet werden.
...
Die Indikatoren und Lernbereiche werden immer aus dem aktuellen Schuljahr in das neue Schuljahr kopiert; das ist besonders wichtig, wenn die Indikatoren vorher manuell editiert wurden.
